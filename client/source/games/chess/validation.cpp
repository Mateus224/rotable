...
int validation::validatePawn(Tile *temp)
{
    ...
    if(temp->pieceColor)
    {
        ...
        {
            exp_[max++]=tile[row-1][col]->tileNum;
            retVal=1;
        }

        if(row==6 && !tile[5][col]->piece && !tile[4][col]->piece)
        {
            exp_[max++]=tile[row-2][col]->tileNum;
            retVal=1;
        }
        ...
        if(row-1>=0 && col-1>=0)
        {
            ...
            {
            exp_[max++]=tile[row-1][col-1]->tileNum;
            retVal=1;
            }
        }
