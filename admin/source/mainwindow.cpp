...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), _ui(new Ui::MainWindow), _isClosing(false) {
  _ui->setupUi(this);
  ...
  connect(_ui->_toolButtonDown, &QToolButton::clicked, this,
          &MainWindow::onDown);
  connect(_ui->_toolButtonAddUser, &QToolButton::clicked, this,
          &MainWindow::actionAddUser);
...
void MainWindow::clearDebugServerLog() { _ui->_textEditServerLog->clear(); }
...
void MainWindow::restore() {
  ...
  _ui->_splitterDb->restoreGeometry(
      settings.value("tabDatabaseSplitterGeometry").toByteArray());
  _ui->_splitterDb->restoreState(
      settings.value("tabDatabaseSplitterState").toByteArray());
  _ui->_tableViewProducts->restoreGeometry(
      settings.value("tableViewProductsGeometry").toByteArray());

  _ui->_tableViewProducts->horizontalHeader()->setSectionResizeMode(
  ...
  _ui->_tableViewProducts->horizontalHeader()->resizeSection(
      ProductTableModel::name,
      settings.value("tableViewProductsGeometryColumnNameWidth", 150).toInt());
  _ui->_tableViewProducts->horizontalHeader()->resizeSection(
      ProductTableModel::amount,
      settings.value("tableViewProductsGeometryColumnAmountWidth", 75).toInt());
  _ui->_tableViewProducts->horizontalHeader()->resizeSection(
      ProductTableModel::icon,
      settings.value("tableViewProductsGeometryColumnIconWidth", 100).toInt());
  _ui->_tableViewProducts->horizontalHeader()->resizeSection(
      ProductTableModel::price,
      settings.value("tableViewProductsGeometryColumnPriceWidth", 100).toInt());
}

//------------------------------------------------------------------------------

void MainWindow::closeEvent(QCloseEvent *) { _isClosing = true; }
